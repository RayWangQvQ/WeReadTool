...
# ------------vars-----------
repoDir=$(dirname $PWD)
projectName="WeReadTool"
consoleDir=$repoDir/src/$projectName
...
publish_dotnet_dependent() {
    ...
    echo "zip files..."
    cd $publishDir
    zip -q -r $projectName-v$version-dotnet-dependent.zip ./dotnet-dependent/*
    ls -l
...
publish_self_contained() {
    ...
    echo "zip files..."
    cd $publishDir
    zip -q -r $projectName-v$version-$runtime.zip ./$runtime/*
    ls -l
    echo -e "---------publish successfully---------\n\n"
}
...
publish_tencentScf() {
    ...
    zip -r ../$projectName-v$version-tencent-scf.zip ./*
    cd .. && ls
    echo -e "---------publish successfully---------\n\n"
}
